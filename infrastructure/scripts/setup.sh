...
if ! (return 0 2>/dev/null); then
  echo -e "${redColour}[ERROR]${endColour} Usage 'source setup.sh'"
  exit
fi

if state_done SETUP; then
  echo -e "${greenColour}[+]${endColour} The setup has been completed"
  return
fi
...
if ps -ef | grep "$SETUP_SCRIPT" | grep -v grep; then
  echo -e "${yellowColour}[+]${endColour} The $SETUP_SCRIPT is already running.  If you want to restart it then kill it and then rerun."
else
